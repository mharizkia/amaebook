    ...
    <div class="relative container mx-auto mt-9 ml-3">
        <div class="bg-gradient-to-l from-black via-amber-950 to-gray-200 shadow-md rounded-lg p-6">
            <div class="max-w-8xl mx-auto px-2 sm:px-6 lg:px-8"> 
                <h1 class="text-2xl font-bold mb-6 text-black w-auto max-w-lg">{{ $buku->judul }}</h1>
                <div class="absolute top-5 right-5">
                    ...
                    <form action="{{ route('bukus.destroy', $buku->id) }}" method="POST" class="inline-block">
                        @csrf
                        @method('DELETE')
                            <button type="submit" onclick="return confirm('Apakah Anda yakin ingin menghapus buku ini?')" class="inline-flex items-center px-4 py-2 bg-red-500 text-white font-semibold text-xs uppercase rounded-md hover:bg-red-700">
                                Delete
                            </button>
                    </form>
                </div>
